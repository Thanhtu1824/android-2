...
export default function PlaylistPlayerScreen() {
  const { playlistId, songId } = useLocalSearchParams<{ playlistId: string; songId: string }>();

  const soundRef = useRef<Audio.Sound | null>(null);

  // refs để tránh state “cũ”
  ...
  // ✅ Load tracks trong playlist + focus vào songId
  useEffect(() => {
    const load = async () => {
      if (!playlistId) return;
      setLoading(true);

      // join playlist_tracks -> songs
      ...
      if (error) {
        console.log("Load playlist tracks error:", error.message);
        setLoading(false);
        return;
      }

      const list: Track[] = (data ?? [])
        .map((row: any) => row.song)
        .filter((s: any) => !!s?.id);

      setTracks(list);

      const found = list.findIndex((t) => t.id === songId);
  ...
  const onShare = async () => {
    if (!current) return;
    const url = getPublicUrl(current.file_path);
    await Share.share({ message: `${current.title} - ${current.artist}\n${url}` });
  };
  ...
  const loopIcon = loopMode === "off" ? "repeat-outline" : "repeat";
